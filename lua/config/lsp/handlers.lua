...
function M.preview_location(loc, _, _)
    ...
    local lines = vim.api.nvim_buf_get_lines(bufnr, (range.start.line - before), (range['end'].line + after + 1), false)
    ...
    local pos = range.start.line == range["end"].line and { before+1, range.start.character+1, range["end"].character-range.start.character } or { before+1 }
